...
function runTheRegister() {
   ...
   let changeValue = Math.round((cashValue - billValue) * 100) / 100; // Ensure proper rounding
...
function calcChange(changeValue) {
   ...
   changeValue -= bill5Amt * 5; // Fixed incorrect multiplier
   ...
   // Convert remaining change to cents to avoid floating point errors
   ...
   let coin1Amt = changeValue; // Remaining changeValue in cents is now pennies
